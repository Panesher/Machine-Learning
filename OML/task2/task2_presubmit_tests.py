...
def check_equal_histories(test_history, reference_history, atol=1e-3):
    ...
    for key in reference_history.keys():
        ...
        if key != 'time':
            assert_allclose(test_history[key],
                            reference_history[key], atol=atol)
        else:
...
class TestCG(unittest.TestCase):
    ...
    def test_default(self):
        ...
        with Capturing() as output:
            x_sol, message, _ = conjugate_gradients(
                self.matvec, self.b, self.x0)
    ...
    def test_display(self):
        ...
        self.assertLess(
            0, len(output), 'You should print the progress when `display` is True.')

    def test_histories(self):
        x_sol, message, history = conjugate_gradients(
            self.matvec, self.b, self.x0, trace=True)
...
class TestLBFGS(unittest.TestCase):
    ...
    def test_line_search_options(self):
        ...
        lbfgs(self.oracle, self.x0, line_search_options={
    ...
    def test_display(self):
        ...
        self.assertTrue(
            len(output) > 0, 'You should print the progress when `display` is True.')
    ...
    def test_history(self):
        ...
        true_history = dict(grad_norm=grad_norm_steps,
                            time=time_steps, x=x_steps, func=func_steps)
        check_equal_histories(history, true_history)
...
class TestHFN(unittest.TestCase):
    ...
    def test_line_search_options(self):
        """Check if argument `line_search_options` is supported."""
        hessian_free_newton(self.oracle, self.x0, line_search_options={
                            'method': 'Wolfe', 'c1': 1e-4, 'c2': 0.9})
    ...
    def test_quality(self):
        x_min, message, _ = hessian_free_newton(
            self.oracle, self.x0, tolerance=1e-5)
        f_min = self.oracle.func(x_min)
